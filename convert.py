...
def summarize(file_content):
    @function
    def _call_sglang_summarize(s, prompt):
        s += system("You are a helpful assistant specialized in summarizing code.")
        s += user(prompt)
        s += assistant(gen("long_answer", max_tokens=512))
    
    state = _call_sglang_summarize.run(prompt=f'Summarize this code:\n\n{file_content}')
    for m in state.messages():
        print(m["role"], ":", m["content"])
    return state['long_answer']

def highlight(file_content):
    @function
    def _call_sglang_highlight(s, prompt):
        s += system("You are a helpful assistant specialized in highlighting key features of code.")
        s += user(prompt)
    ...
    state = _call_sglang_highlight.run(prompt=f'Highlight the key features of this code:\n\n{file_content}. What would you say is the key thing to look for for this code?')
    for m in state.messages():
        print(m["role"], ":", m["content"])
...
def overall_summarize(file_content):
    ...
    def _call_sglang_overall_summarize(s, prompt):
        s += system("You are a helpful assistant specialized in providing overall summaries of codebases.")
        s += user(prompt)
    ...
    state = _call_sglang_overall_summarize.run(prompt=f'Provide an overall summary of this codebase:\n\n{file_content}')
    ret = state.text().split("### Response:\nSure. There are multiple methods and classes.\n")[-1]
    return ret

def high_level_pseudocode(file_content):
    @function
    def _call_sglang_high_level_pseudocode(s, prompt):
        s += system("You are a helpful assistant specialized in generating high-level pseudocode.")
        s += user(prompt)
        s += "1. List all the major, important methods and functions, and rate their importance.\n"
        s += "2. Describe what this file is all about.\n"
        s += "### Response:\nSure. Here is the pythonic pseudocode that overviews the file you described.\n"
        s += "```python\n"
        s += gen("long_answer", max_tokens = 2048, stop = '```')
    
    state = _call_sglang_high_level_pseudocode.run(prompt=f'Generate high-level pseudocode for this code:\n\n{file_content}')
    for m in state.messages():
        print(m["role"], ":", m["content"])
...
def analyze_directory_and_generate_overall_json(this_directory_path, output_directory, global_codebase_path):
    all_summaries = []
    ...
    for root, _, files in os.walk(this_directory_path):
        for file in files:
            if not file.endswith('.json'):
                ...
                analyze_file_and_generate_json(file_path, output_directory, global_codebase_path)
                relative_file_path = file_path.relative_to(this_directory_path)
                json_file_path = output_directory / relative_file_path.with_suffix('.json')
    ...
    # for item in os.listdir(this_directory_path):
    #     item_path = Path(this_directory_path, item)
    #     if item_path.is_dir():
    #         analyze_directory_and_generate_overall_json(item_path, output_directory, global_codebase_path)
...
def main(codebase_dir, output_directory, port):
    ...
    analyze_directory_and_generate_overall_json(codebase_dir, output_directory, codebase_dir)
